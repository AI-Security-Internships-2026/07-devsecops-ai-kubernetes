...
python run.py pipeline nginx:latest --falco falco_alerts.json          # + Falco runtime reachability
python run.py pipeline nginx:latest --gatekeeper                       # + emit OPA Gatekeeper YAML
```
...
**Cluster, assistant, policy, and runtime signals:**

```bash
python run.py discover                                         # inventory cluster pods (image/status/exposure)
...
**Current state:** the core triage pipeline (Trivy → EPSS → CISA KEV → SSVC → report), the CVE-intelligence database (NVD/OSV pollers, SBOM ingestion, matcher, LangGraph watcher agent), Kubernetes deployment context, and the Exploit-DB signal are all merged through Week 6. Week 7 added the seven MCP servers + interactive CLI chatbot, OPA Gatekeeper policy generation, and Falco runtime-alert capture. Week 8 wires Falco (runtime reachability) and Gatekeeper into the main pipeline, adds a cluster `discover` command, a background watcher service (`watch --status`/`--stop`), and lets the chat assistant read saved triage reports.
